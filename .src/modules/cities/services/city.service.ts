...
import { CityEntity } from '../entity/city.entity';
  ...
  export class CityService {
    constructor(
    @Inject(REPOSITORIES.City)
    private readonly _CityRepository: typeof CityEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: CityService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._CityRepository.sequelize.query(
          CityTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._CityRepository.sequelize.query(
        CityQuery (fieldSearch),
        {
          replacements: {
