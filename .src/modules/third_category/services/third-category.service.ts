...
import { ThirdCategoryEntity } from '../entity/third-category.entity';

  @Injectable()
  export class ThirdCategoryService {
    constructor(
    @Inject(REPOSITORIES.ThirdCategory)
    private readonly _ThirdCategoryRepository: typeof ThirdCategoryEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: ThirdCategoryService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._ThirdCategoryRepository.sequelize.query(
          ThirdCategoryTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._ThirdCategoryRepository.sequelize.query(
        ThirdCategoryQuery (fieldSearch),
        {
          replacements: {
