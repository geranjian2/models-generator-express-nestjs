...
import { TeamEntity } from '../entity/team.entity';
  ...
  export class TeamService {
    constructor(
    @Inject(REPOSITORIES.Team)
    private readonly _TeamRepository: typeof TeamEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: TeamService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._TeamRepository.sequelize.query(
          TeamTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._TeamRepository.sequelize.query(
        TeamQuery (fieldSearch),
        {
          replacements: {
