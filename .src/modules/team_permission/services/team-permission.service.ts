...
import { TeamPermissionEntity } from '../entity/team-permission.entity';

  @Injectable()
  export class TeamPermissionService {
    constructor(
    @Inject(REPOSITORIES.TeamPermission)
    private readonly _TeamPermissionRepository: typeof TeamPermissionEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: TeamPermissionService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._TeamPermissionRepository.sequelize.query(
          TeamPermissionTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._TeamPermissionRepository.sequelize.query(
        TeamPermissionQuery (fieldSearch),
        {
          replacements: {
