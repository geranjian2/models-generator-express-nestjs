...
import { ThirdEntity } from '../entity/third.entity';

  @Injectable()
  export class ThirdService {
    constructor(
    @Inject(REPOSITORIES.Third)
    private readonly _ThirdRepository: typeof ThirdEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: ThirdService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._ThirdRepository.sequelize.query(
          ThirdTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._ThirdRepository.sequelize.query(
        ThirdQuery (fieldSearch),
        {
          replacements: {
