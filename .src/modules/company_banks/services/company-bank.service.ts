...
import { CompanyBankEntity } from '../entity/company-bank.entity';

  @Injectable()
  export class CompanyBankService {
    constructor(
    @Inject(REPOSITORIES.CompanyBank)
    private readonly _CompanyBankRepository: typeof CompanyBankEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: CompanyBankService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._CompanyBankRepository.sequelize.query(
          CompanyBankTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._CompanyBankRepository.sequelize.query(
        CompanyBankQuery (fieldSearch),
        {
          replacements: {
