...
import { InvoicePaymentEntity } from '../entity/invoice-payment.entity';

  @Injectable()
  export class InvoicePaymentService {
    constructor(
    @Inject(REPOSITORIES.InvoicePayment)
    private readonly _InvoicePaymentRepository: typeof InvoicePaymentEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: InvoicePaymentService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._InvoicePaymentRepository.sequelize.query(
          InvoicePaymentTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._InvoicePaymentRepository.sequelize.query(
        InvoicePaymentQuery (fieldSearch),
        {
          replacements: {
