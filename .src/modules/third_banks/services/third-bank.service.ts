...
import { ThirdBankEntity } from '../entity/third-bank.entity';

  @Injectable()
  export class ThirdBankService {
    constructor(
    @Inject(REPOSITORIES.ThirdBank)
    private readonly _ThirdBankRepository: typeof ThirdBankEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: ThirdBankService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._ThirdBankRepository.sequelize.query(
          ThirdBankTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._ThirdBankRepository.sequelize.query(
        ThirdBankQuery (fieldSearch),
        {
          replacements: {
