...
import { CompanyTeamEntity } from '../entity/company-team.entity';

  @Injectable()
  export class CompanyTeamService {
    constructor(
    @Inject(REPOSITORIES.CompanyTeam)
    private readonly _CompanyTeamRepository: typeof CompanyTeamEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: CompanyTeamService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._CompanyTeamRepository.sequelize.query(
          CompanyTeamTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._CompanyTeamRepository.sequelize.query(
        CompanyTeamQuery (fieldSearch),
        {
          replacements: {
