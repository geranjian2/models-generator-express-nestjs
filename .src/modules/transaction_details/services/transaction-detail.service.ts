...
import { TransactionDetailEntity } from '../entity/transaction-detail.entity';

  @Injectable()
  export class TransactionDetailService {
    constructor(
    @Inject(REPOSITORIES.TransactionDetail)
    private readonly _TransactionDetailRepository: typeof TransactionDetailEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: TransactionDetailService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._TransactionDetailRepository.sequelize.query(
          TransactionDetailTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._TransactionDetailRepository.sequelize.query(
        TransactionDetailQuery (fieldSearch),
        {
          replacements: {
