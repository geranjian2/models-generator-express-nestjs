...
import { CompanyPlanEntity } from '../entity/company-plan.entity';

  @Injectable()
  export class CompanyPlanService {
    constructor(
    @Inject(REPOSITORIES.CompanyPlan)
    private readonly _CompanyPlanRepository: typeof CompanyPlanEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: CompanyPlanService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._CompanyPlanRepository.sequelize.query(
          CompanyPlanTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._CompanyPlanRepository.sequelize.query(
        CompanyPlanQuery (fieldSearch),
        {
          replacements: {
