...
import { RolePermissionEntity } from '../entity/role-permission.entity';
  ...
  export class RolePermissionService {
    constructor(
    @Inject(REPOSITORIES.RolePermission)
    private readonly _RolePermissionRepository: typeof RolePermissionEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: RolePermissionService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._RolePermissionRepository.sequelize.query(
          RolePermissionTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._RolePermissionRepository.sequelize.query(
        RolePermissionQuery (fieldSearch),
        {
          replacements: {
