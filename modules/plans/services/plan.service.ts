...
import { PlanEntity } from '../entity/plan.entity';
  ...
  export class PlanService {
    constructor(
    @Inject(REPOSITORIES.Plan)
    private readonly _PlanRepository: typeof PlanEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: PlanService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._PlanRepository.sequelize.query(
          PlanTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._PlanRepository.sequelize.query(
        PlanQuery (fieldSearch),
        {
          replacements: {
