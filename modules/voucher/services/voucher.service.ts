...
import { VoucherEntity } from '../entity/voucher.entity';
  ...
  export class VoucherService {
    constructor(
    @Inject(REPOSITORIES.Voucher)
    private readonly _VoucherRepository: typeof VoucherEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: VoucherService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._VoucherRepository.sequelize.query(
          VoucherTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._VoucherRepository.sequelize.query(
        VoucherQuery (fieldSearch),
        {
          replacements: {
