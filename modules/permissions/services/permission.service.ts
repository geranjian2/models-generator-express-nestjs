...
import { PermissionEntity } from '../entity/permission.entity';
  ...
  export class PermissionService {
    ...
    private readonly _PermissionRepository: typeof PermissionEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: PermissionService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._PermissionRepository.sequelize.query(
          PermissionTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._PermissionRepository.sequelize.query(
        PermissionQuery (fieldSearch),
        {
          replacements: {
