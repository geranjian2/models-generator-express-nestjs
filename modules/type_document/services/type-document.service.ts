...
import { TypeDocumentEntity } from '../entity/type-document.entity';

  @Injectable()
  export class TypeDocumentService {
    constructor(
    @Inject(REPOSITORIES.TypeDocument)
    private readonly _TypeDocumentRepository: typeof TypeDocumentEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: TypeDocumentService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._TypeDocumentRepository.sequelize.query(
          TypeDocumentTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._TypeDocumentRepository.sequelize.query(
        TypeDocumentQuery (fieldSearch),
        {
          replacements: {
