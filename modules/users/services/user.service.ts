...
import { UserEntity } from '../entity/user.entity';
  ...
  async pagination(
    ...
    const log = Log({ module: UserService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._UserRepository.sequelize.query(
          UserTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._UserRepository.sequelize.query(
        UserQuery (fieldSearch),
        {
          replacements: {
