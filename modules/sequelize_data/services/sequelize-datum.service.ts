...
import { SequelizeDatumEntity } from '../entity/sequelize-datum.entity';

  @Injectable()
  export class SequelizeDatumService {
    constructor(
    @Inject(REPOSITORIES.SequelizeDatum)
    private readonly _SequelizeDatumRepository: typeof SequelizeDatumEntity,
  ...
  async pagination(
    ...
    const log = Log({ module: SequelizeDatumService.name, method: 'pagination' });
    try {
     const offset = (page - 1) * pageSize;
     const count_ = (
        (await this._SequelizeDatumRepository.sequelize.query(
          SequelizeDatumTotalQuery(fieldSearch),
          {
            replacements: {
      ...
      const data = (await this._SequelizeDatumRepository.sequelize.query(
        SequelizeDatumQuery (fieldSearch),
        {
          replacements: {
